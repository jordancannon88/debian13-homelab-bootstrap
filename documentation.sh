...
det_ip="$(ip -4 route get 1.1.1.1 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="src"){print $(i+1); exit}}' || true)"
...
det_port="$(sshd -T 2>/dev/null | awk '$1=="port"{print $2; exit}' || true)"
[[ -z "$det_port" ]] && det_port="$(awk 'tolower($1)=="port"{print $2; exit}' /etc/ssh/sshd_config 2>/dev/null || true)"
...
if [[ "$det_user" == "root" ]]; then
  _alt="$(awk 'NF{print; exit}' /var/lib/homelab-bootstrap/created-users 2>/dev/null || true)"
  [[ -z "$_alt" ]] && _alt="$(getent group sudo 2>/dev/null | cut -d: -f4 | tr ',' '\n' | awk 'NF{print; exit}' || true)"
  [[ -n "$_alt" ]] && det_user="$_alt"
fi
det_alias="${det_fqdn%%.*}"
det_os="$( . /etc/os-release 2>/dev/null; echo "${PRETTY_NAME:-Linux}" )"
det_shell="$(getent passwd "$det_user" 2>/dev/null | cut -d: -f7 || true)"
det_shell="${det_shell##*/}"; det_shell="${det_shell:-bash}"
...
det_root_ssh="$(awk 'tolower($1)=="permitrootlogin"{print tolower($2); exit}' /etc/ssh/sshd_config 2>/dev/null || true)"
...
<p class="subtitle">How to reach <code>${e_alias}</code> and open an SSH session on its hardened port.</p>

<div class="callout info">
  <strong>SSH is not on port 22.</strong> <code>harden.sh</code> moves the daemon to a non-standard
  port and the nftables firewall is deny-by-default, so only the configured SSH port is reachable.
  On this host that port is <code>${e_port}</code>. Every example below sets it explicitly.
</div>

<div class="callout warning">
  <strong>Key-only login.</strong> Password authentication is disabled and root SSH is off.
  You need the private key matching <code>~/.ssh/${e_key}</code> on your local machine; there is
  no password fallback. If the key is lost, recovery is the console (Proxmox/VM console or
  physical), not SSH.
</div>
...
<h2 id="server">The ${e_alias} server</h2>
<table>
  ...
  <tr><th>Login shell</th><td><code>${e_shell}</code></td></tr>
  <tr><th>Root access</th><td>${e_root}</td></tr>
  <tr><th>SSH key</th><td><code>~/.ssh/${e_key}</code></td></tr>
</table>
...
<div class="callout success">
  <strong>Want passthrough args?</strong> Write it as a function instead, so you can append flags
  like a tunnel: <code>${e_alias} -L 3000:localhost:3000</code>.
</div>
...
<div class="callout info">
  ...
  In bash or zsh use <code>~/.ssh/config</code> (next section) instead, which every SSH tool inherits.
</div>

<h2 id="config">SSH client config</h2>
...
<p>Connection times out or is refused? First verify the host is up and the port is reachable:</p>
...
<div class="callout warning">
  <strong>Suddenly locked out after a few failed tries?</strong> fail2ban bans an IP for one hour after
  5 failed SSH logins within 10 minutes. Wait it out, or from the server's console run
  <code>fail2ban-client set sshd unbanip &lt;your-ip&gt;</code>.
</div>
