...
# Secrets live in /etc/seafile-keeper/env (root, 0600), plain key=value, no quoting:
...
# Plain key=value lines, read literally: no shell expansion, so a password with
# $ or quotes needs no escaping. Everything after the first = is the value.
SEAFILE_URL=""; SEAFILE_TOKEN=""; REPO_ID=""; REPO_PASSWORD=""
while IFS= read -r line || [ -n "$line" ]; do
  case "$line" in ''|'#'*) continue;; esac
  k=${line%%=*}; v=${line#*=}
  case "$k" in SEAFILE_URL|SEAFILE_TOKEN|REPO_ID|REPO_PASSWORD) printf -v "$k" '%s' "$v";; esac
done < "$ENV"
for k in SEAFILE_URL SEAFILE_TOKEN REPO_ID REPO_PASSWORD; do
  [ -n "${!k}" ] || { echo "$ENV: $k is empty" >&2; exit 2; }
done
mkdir -p "$STATE"
