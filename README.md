...
| `AIDE_EXCLUDES="/mnt /media /export /var/lib/vz"` | Paths AIDE never indexes (bulk-data mounts such as NFS datastores and docked disks, and PVE guest disk images under `/var/lib/vz`); on ZFS hosts every pool mountpoint other than `/` is added automatically (container subvols, replica pools); on docker/podman hosts `/var/lib/docker`, `/var/lib/containers` and every `~/.local/share/docker` too. App data only the host knows about (recordings, media) goes in a hand-written `/etc/aide/aide.conf.d/98_local_exclude` with the same `-/path` lines, which the bootstrap never touches |
