...
| `AIDE_EXCLUDES="/mnt /media /export"` | Paths AIDE never indexes (bulk-data mounts such as NFS datastores and docked disks) |
...
| `AIDE_EXCLUDES="/mnt /media /export"` | Paths AIDE never indexes, written to `/etc/aide/aide.conf.d/99_homelab_exclude` as non-recursive `-<path>` rules (AIDE 0.19+; the classic `!` form still walks the tree). Default covers the usual bulk-data mount points; indexing an NFS datastore there once cost a host 16 h of CPU |
